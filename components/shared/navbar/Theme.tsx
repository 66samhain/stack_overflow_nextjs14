  ...
  return (
    <Menubar className="relative border-none bg-transparent shadow-none">
      <MenubarMenu>
        ...
        <MenubarContent
          className="absolute right-[-3rem] mt-3 min-w-[120px] rounded border bg-white py-2 dark:border-dark-400 dark:bg-dark-300">
          {themes.map(item => (
            <MenubarItem
              key={item.value}
              ...
              onClick={() => {
                ...
                if (item.value !== 'system') {
                  ...
                  localStorage.removeItem('theme'); // if we use the system theme, remove preference from local storage
                }
              }}
            >
              <Image
                src={item.icon} alt={item.value} width={16} height={16}
